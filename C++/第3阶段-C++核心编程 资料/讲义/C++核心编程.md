...
public:
	...
	Person& PersonAddPerson(Person p) //使用引用的方式接受返回的对象
	{
		this->age += p.age;
		//返回对象本身
		return *this;
	}
  //Person PersonAddPerson(Person p) //使用值的方式接受返回的对象，此时是对象复制的形式，结果为2
	//{
	//	this->age += p.age;
		//返回对象本身
	//	return *this;
...
public:
	...
	//后置++ 注意返回值
	MyInteger operator++(int) { //使用的int为占位参数，只能使用int 
		//先返回
...
<img src="C:\Users\ghdyx\AppData\Roaming\Typora\typora-user-images\image-20210531092923478.png" alt="image-20210531092923478" style="zoom: 67%;" />

总结：

* 菱形继承带来的主要问题是子类继承两份相同的数据，导致资源浪费以及毫无意义
* 利用虚继承可以解决菱形继承问题
* 虚继承实际上是继承的指针，根据指针进行的额地址偏移查找
